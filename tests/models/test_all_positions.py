...
class TestAllPositions:
    def test_no_extra_request(self):
        data = AllPositions(source_app="travis")
    ...
    def test_start_time_extraction(self):
        ...
        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", start_time=START_TIME)
    ...
    def test_end_time_extraction(self):
        ...
        # Both value must be set or unset
        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", end_time=END_TIME)
    ...
    def test_start_coordinates_extraction(self):
        ...
        # Both value must be set or unset
        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", start_lat=START_LAT)

        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", start_lon=START_LON)

        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", start_lat=START_LAT, start_lon=START_LON)
    ...
    def test_end_coordinates_extraction(self):
        ...
        # Both value must be set or unset
        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", end_lat=END_LAT)

        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", end_lon=END_LON)

        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", end_lat=END_LAT, end_lon=END_LON)

        with pytest.raises(ValidationError):
            AllPositions(source_app="travis", end_lon=END_LON, end_radius=END_RADIUS)
    ...
    def test_type_detection_extraction(self):
        ...
        data = AllPositions(source_app="travis", type_mobility=TYPE_MOBILITY)
        assert f"""'{TYPE_MOBILITY}' = "type" """ in data._query_external
        assert data._query_type_detection_extraction is None

        data = AllPositions(source_app="travis", type_detection=TYPE_DETECTION)
        assert data._query_type_detection_extraction
