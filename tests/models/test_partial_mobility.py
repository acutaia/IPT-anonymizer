...
class TestPartialMobility:
    ...
    def test_type_detection_extraction(self):
        data = PartialMobility(type_mobility=TYPE_MOBILITY, type_aggregation=TYPE_AGGREGATION, type_detection=TYPE_DETECTION)
        assert f"{data._query_type_mobility_extract} AND {data._query_type_detection_extraction}" in data._query_external
