...
class TestPartialMobility:
    def test_no_extra_request(self):
        data = PartialMobility(source_app="travis")
    ...
    def test_start_time_extraction(self):
        ...
        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", start_time=START_TIME)
    ...
    def test_end_time_extraction(self):
        ...
        # Both value must be set or unset
        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", end_time=END_TIME)
    ...
    def test_start_coordinates_extraction(self):
        ...
        # Both value must be set or unset
        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", start_lat=START_LAT)

        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", start_lon=START_LON)
    ...
    def test_end_coordinates_extraction(self):
        ...
        # Both value must be set or unset
        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", end_lat=END_LAT)

        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", end_lon=END_LON)

        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", end_lat=END_LAT, end_lon=END_LON)

        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", end_lon=END_LON, end_radius=END_RADIUS)
    ...
    def test_partial_mobility_type_extraction(self):
        ...
        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", type_mobility=TYPE_MOBILITY)

        with pytest.raises(ValidationError):
            PartialMobility(source_app="travis", type_aggregation=TYPE_AGGREGATION)
    ...
    def test_type_detection_extraction(self):
        ...
        assert (
            f"{data._query_type_mobility_extract} AND {data._query_type_detection_extraction}"
            in data._query_external
        )

        data = PartialMobility(source_app="travis", type_detection=TYPE_DETECTION)
        assert data._query_type_detection_extraction in data._query_external
